...
func (ty *SingleConfig) InitHttp() {
	...
	ty.E.Use(session.Middleware(
		sessions.NewCookieStore([]byte("makewebtool")))) // session store생성함
	...
	template := &Template{
		templates: template.Must(template.ParseGlob("router/public/views/*.html")),
	}

	ty.E.Static("/", "./router/public/assets")
	ty.E.Renderer = template
